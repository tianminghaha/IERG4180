...
struct thread_data {
	...
	double jitter = 0;
	double average = 0;
	clock_t updateTime = 0; //time interval to update status
	int packetSize = 0;
	int packaccu = 0; //number of total received packages.
	int packno = 0; //number of total sent packages.
...
int main(int argc, char *argv[])
{
	...
	clock_t updateTime = DEFAULT_UPDATE_TIME;
	...
	if (mode.compare("-host") == 0) {
		...
		if (protocol.compare("UDP") == 0) {
			...
			while (1)
			{
				...
				int bytes_recv = 0;
				while (bytes_recv < pktSize) {
					int retVal = recv(newsfd, buffer, pktSize, 0);
					if (retVal > 0) {
						bytes_recv += retVal;
					}
					else {
		...
		if (protocol.compare("UDP") == 0) {
			...
			while (pktNum == 0 || (pktNum != 0 && numSent <= pktNum))
			{
				...
				numSent++;
				clock_t endTime = clock();
				clock_t targetTime = beforeTime + (pktSize / pktRate) * 1000;
...
DWORD WINAPI SendThread(LPVOID lpParam) {
	...
	while (true) {
		...
		if (currentTime == 0 || currentTime == startTime) {
			...
			elapse = (currentTime - startTime) / (double)CLOCKS_PER_SEC;
		}
...
DWORD WINAPI RecvThread(LPVOID lpParam) {
	...
	while (true) {
		...
		else { lostRate = (double)lostNum / (lostNum + numPackrec); }
		if (currentTime == 0 || currentTime == startTime) {
			continue;
		}
		else {
		...
		printf("\rElapsed[%ds] Pkts[%d] Lost[%d, %.2f%%] Rate[%.5fKbps] Jitter[%.2fms]", (int)elapse, numPackrec, lostNum, lostRate, throughPut, jitter);
	}
	return 0;
}